...
export const ModalAddProducts = (props) => {
    ...
    const handleAdd = async (e) => {
        e.preventDefault()
        if(!formProducts.name){
            alert("jumlah diamond wajib diisi!")
        }
        if(!formProducts.game){
            alert("tipe game wajib diisi!")
        }
        if(!formProducts.price){
            alert("harga wajib diisi!")
        }
        try {
            const result = await PostProducts({
                name : formProducts.name,
                game : formProducts.game,
    ...
    return(
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
            ...
            <h1 className="text-3xl font-bold text-white text-center mb-8">Tambah Produk</h1>
                <div className="space-y-5">
                    ...
                    <div className="flex gap-4 pt-3">
                        <button className="flex-1 bg-[#7A1F2B] hover:bg-[#962D3E] transition-all duration-300 rounded-xl py-3 font-semibold text-white" onClick={handleAdd}>
                            Simpan
                        </button>
